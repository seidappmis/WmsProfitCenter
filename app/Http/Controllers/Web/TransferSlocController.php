...
class TransferSlocController extends Controller
{
  ...
  {
	$prev_quantity	= $request->input('prev_quantity');
	$quantity		= $request->input('qty');

	if ($quantity > $prev_quantity) {
		throw new Exception("Available Quantity tidak mencukupi", 500);
	} else {
		return DB::transaction(function () use ($request, $quantity) {
