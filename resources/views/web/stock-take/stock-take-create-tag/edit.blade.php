                ...
                <div class="card">
                    <div class="card-content p-0">
                      <ul class="collapsible m-0">
                        <li class="active">
                          <div class="collapsible-header"><i class="material-icons">keyboard_arrow_right</i>Edit Input Tag</div>
                          <div class="collapsible-body">
                            <form class="form-table" id="form-input-manual-tag">
                              <input type="hidden" name="sto_id" value="{{$schedule->sto_id}}">
                              <table>
                                <tr>
                                  <td width="30%">No Tag</td>
                                  <td>
                                    <div class="input-field col s12">
                                      <input type="hidden" name="no_tag_old" value="{{$input->no_tag}}">
                                      <input type="text" name="no_tag" value="{{$input->no_tag}}">
                                    </div>
                                  </td>
                                </tr>
                                <tr>
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function($) {
    set_select2_value('#form-input-manual-tag [name="model_id"]', '{{$input->model}}', '{{$input->model}}')
    set_select2_value('#form-input-manual-tag [name="location"]', '{{$input->location}}', '{{$input->location}}')
    $('#form-input-manual-tag [name="model"]').val('{{$input->model}}');
    ...
    $('#form-input-manual-tag').validate({
      submitHandler: function(form){
        setLoading(true)
        $.ajax({
          url: "{{url('stock-take-create-tag/' . $schedule->sto_id . '/edit/' . $input->no_tag)}}",
          type: 'PUT',
          dataType: 'json',
          data: $(form).serialize(),
        })
